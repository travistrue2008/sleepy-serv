...
async function buildHandlers (route) {
...
async function buildRoutes (rootPath, mountPath) {
  const metadata = buildRoutesMetadata(rootPath, mountPath)
  const moduleRoutes = await Promise.all(metadata.map(buildHandlers))
...
export async function createServer (port, rootPath, opts = {}) {
  ...
  const routes = await buildRoutes(`${rootPath}/api`, mountPath)
