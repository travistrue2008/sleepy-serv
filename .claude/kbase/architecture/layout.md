...
- `packages/server/`: the published npm package `sleepy-serv` (nearly all work happens here). ESM, multiple exports (`.`, `./plugin`, `./plugin/builder`). Source split into `src/core/` (engine), `src/plugin/` (route discovery), and `src/index.ts` (main entry point).
