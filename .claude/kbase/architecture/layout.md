...
- `packages/server/`: the published npm package `sleepy-serv` (nearly all work happens here). ESM, exports `.` and `./plugin`, `bin` for the `sleepy` CLI. Source split into `src/core/` (engine), `src/plugin/` (scanner, codegen, config), `src/cli/` (init, dev, build commands), and `src/index.ts` (main entry point with type declarations).
